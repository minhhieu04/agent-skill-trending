...
export const IntroScene: React.FC<IntroSceneProps> = ({
  ...
  const tags = ['🤖 AI Agent', '⚡ Productivity', '🔥 GitHub Trending', '🧠 LLM', '🛡️ Sandboxed'];
  ...
  return (
    <AbsoluteFill style={{ overflow: 'hidden', backgroundColor: '#090d16', opacity: fadeOut }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: isVertical ? '9px' : '11px', fontWeight: 700, color: '#fbbf24', letterSpacing: '0.1em' }}>TOP RANK</span>
        <span style={{ fontSize: isVertical ? '18px' : '24px', fontWeight: 900, color: '#fff' }}>#1</span>
      </div>
      ...
      <div style={{
        ...
        {skillStats && (
          <div style={{
            ...
            {[
              { icon: '⭐', label: `${(skillStats.stars || 4280).toLocaleString()} Stars`, bg: 'rgba(245,158,11,0.15)', border: 'rgba(245,158,11,0.4)', color: '#fbbf24' },
              { icon: '🍴', label: `${(skillStats.forks || 340).toLocaleString()} Forks`, bg: 'rgba(99,102,241,0.15)', border: 'rgba(99,102,241,0.4)', color: '#a5b4fc' },
              { icon: '⚡', label: skillStats.language || 'TypeScript', bg: 'rgba(16,185,129,0.15)', border: 'rgba(16,185,129,0.4)', color: '#6ee7b7' },
            ].map((s, i) => (
              <div key={i} style={{
