...
export const ExportModal: React.FC<ExportModalProps> = ({ skill, isOpen = true, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-4xl max-h-[90vh] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95"
      ...
      >
        ...
        {/* IDE Selector Tabs */}
        <div className="px-6 pt-3 border-b border-slate-200 dark:border-slate-800 flex gap-2 overflow-x-auto bg-slate-50/50 dark:bg-slate-950/40">
          {ideOptions.map((ide) => (
            <button
              key={ide.id}
              onClick={() => setActiveIde(ide.id)}
              className={`px-4 py-3 border-b-2 font-medium text-xs whitespace-nowrap transition-all flex items-center gap-2 ${
                activeIde === ide.id
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-100/80 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-xs font-mono text-slate-700 dark:text-slate-300">
                  <span className="text-slate-400">{t('target_file')}:</span>
                  <strong className="text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-md border border-emerald-500/20">
                    {config.file_path}
                  </strong>
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-emerald-500 text-xs font-bold transition-all shadow-sm"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-md shadow-emerald-600/20"
                  >
              ...
              {config.cli_command && (
                <div className="p-3.5 rounded-2xl bg-slate-900 border border-slate-800 text-slate-200 space-y-1.5">
                  ...
                  <pre className="text-xs font-mono text-emerald-300 overflow-x-auto whitespace-pre-wrap select-all">
                    {config.cli_command}
                  </pre>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <pre className="p-5 rounded-2xl bg-slate-900 dark:bg-slate-950 border border-slate-800 text-xs font-mono text-slate-100 overflow-x-auto max-h-72 leading-relaxed shadow-inner">
                  {config.content}
                </pre>
              </div>
            </div>
